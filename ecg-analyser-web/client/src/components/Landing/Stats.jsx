...
const stats = [
  ...
  { icon: FileText, value: '60+', label: 'Reports Generated', color: 'from-green-500 to-emerald-500' },
...
function StatCard({ stat, visible, index }) {
  ...
  const animatedValue = useCountUp(stat.value, 2000, visible);

  return (
    <div className={`card-hover p-8 text-center group transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{ transitionDelay: `${index * 150}ms` }}>
      ...
      <div className="text-3xl md:text-4xl font-bold gradient-text mb-1">{animatedValue}</div>
    ...
    <section id="stats" className="py-24 relative bg-[var(--bg-primary)]">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((stat, i) => (
            <StatCard key={i} stat={stat} visible={visible} index={i} />
          ))}
        </div>

        <div className={`mt-16 glass-card rounded-2xl p-8 md:p-12 text-center transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{ transitionDelay: '600ms' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="/register" className="btn-primary inline-flex items-center justify-center gap-2 group">
              Get Started Free
            </a>
